 ...
 * 
 * userId_fk : 1,
 sectorID_fk : $("#sct_id").val(),
 stockID_fk:  $("#stk_id").val(),
 stockAmount: $("#st_amount").val(),
 isBuy:0	
 */
...
$(document).ready(function () {
    ...
    ajaxGet();
    $('#sellBroker').submit(function (event) {
        ...
        else if(parseInt($('#st_amount').val())>parseInt($('#stkAmount').val()))
        {
            alert("Stock amount is not sufficient!");
        }
        else{
   ...
   document.getElementById('st_amount').onkeydown= function(e) {
      ...
      $('#profit').val($('#sellRate').val()*$('#st_amount').val());
 ...
 function ajaxGetStocksNew() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                stockList = data;
                $('#stk_id').empty();
                     $('#stk_id').append('<option value=""> </option>');
                for (ii in stockList) {
                    $('#stk_id').append('<option value="' + stockList[ii].id + '">' + stockList[ii].stockName + '</option>');
